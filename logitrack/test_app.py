...
# ==========================================
# CASO 28: EXPORTACIÓN CSV - ANALÍTICA (US-15)
# ==========================================

def test_boton_exportar_visible_solo_supervisor(client):
    """Prueba AC1: El botón de exportar solo aparece para el Supervisor"""
    # 1. Login como Supervisor
    client.post('/login', data={'usuario': 'supervisor', 'password': 'sup123'}, follow_redirects=True)
    res_sup = client.get('/envios', follow_redirects=True)
    
    # El botón debe existir (buscamos por ID o clase común de exportación)
    assert "Exportar" in res_sup.data.decode('utf-8')
    assert "btn-export" in res_sup.data.decode('utf-8') or "csv" in res_sup.data.decode('utf-8').lower()

    # 2. Login como Operador
    client.post('/logout', follow_redirects=True)
    client.post('/login', data={'usuario': 'operador', 'password': 'ope123'}, follow_redirects=True)
    res_ope = client.get('/envios', follow_redirects=True)
    
    # El botón NO debe estar para el operador
    assert "btn-export" not in res_ope.data.decode('utf-8')

def test_anonimizacion_datos_en_tabla(client):
    """Prueba AC3/AC4: Verifica que los datos sensibles no estén expuestos en la vista de exportación"""
    # Si el JS va a sacar los datos de la tabla, la tabla no debe tener datos prohibidos
    client.post('/login', data={'usuario': 'supervisor', 'password': 'sup123'}, follow_redirects=True)
    respuesta = client.get('/envios', follow_redirects=True)
    texto_html = respuesta.data.decode('utf-8')
    
    # Verificamos que no haya columnas de "DNI" o "Teléfono" que el JS podría leer por error
    assert "DNI" not in texto_html
    assert "Teléfono" not in texto_html
    # Pero sí deben estar los datos operativos
    assert "Tracking" in texto_html
    assert "Estado" in texto_html

def test_exportar_con_filtros_activos(client):
    """Prueba AC2: El sistema responde correctamente a rutas filtradas para exportar"""
    client.post('/login', data={'usuario': 'supervisor', 'password': 'sup123'}, follow_redirects=True)
    
    # Simulamos un filtro por estado 'Entregado'
    respuesta = client.get('/envios?estado=Entregado', follow_redirects=True)
    texto_html = respuesta.data.decode('utf-8')
    
    assert respuesta.status_code == 200
    # Verificamos que la página cargue los datos filtrados para que el JS los tome
    assert "Entregado" in texto_html

def test_exportar_tabla_vacia_deshabilitado(client):
    """Prueba AC6: Manejo de casos donde no hay datos para exportar"""
    client.post('/login', data={'usuario': 'supervisor', 'password': 'sup123'}, follow_redirects=True)
    
    # Buscamos un filtro que sabemos que dará vacío (ej: un ID inexistente)
    respuesta = client.get('/envios?search=ID_IMPOSIBLE_123', follow_redirects=True)
    texto_html = respuesta.data.decode('utf-8')
    
    # Verificamos que el sistema maneje el estado vacío (clase disabled o mensaje)
    assert "No se encontraron" in texto_html or "disabled" in texto_html
